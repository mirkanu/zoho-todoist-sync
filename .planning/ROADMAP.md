...
**Plans**: 4 plans
  - [x] 08-01-PLAN.md — Health endpoint (`app/health/router.py`) with OBS-1 response shape, D-10 status logic, HTTP 200/503 mapping; mounted in `app/main.py`
  - [x] 08-02-PLAN.md — Daily summary arq cron (`app/worker/daily_summary.py`) at midnight UTC: 90-day cleanup → counts → create + complete Todoist summary task; registered in `WorkerSettings.cron_jobs`
  - [x] 08-03-PLAN.md — Migration script (`scripts/migrate.py`) with `--dry-run`: links existing Make.com pairs into sync_state, replaces description with footer (SEED-3), 404 fallback per D-03
